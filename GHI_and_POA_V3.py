...
def write_excel_sheet(wb, sheet_name, date_val, records):
    ...
    for r in range(num_rows):
        ...
        # Min / Max / Delta - Only if Type is NOT GHI (since GHI has only 1 val usually)
        # Actually, formula handles 1 value fine, but Delta is 0. 
        # But per your request "Leaves out GHI values where there are none"
        # We can detect if it's GHI in Python, OR just use excel logic COUNT.
        
        # Better: Standard formulas work, but let's strictly follow the visual style
        # which implies we only check deviation if there are >1 values.
        ...
        # Delta: =MAX - MIN
        # We use address relative to current cell
        col_min_char = chr(65 + col_idx_min)
        col_max_char = chr(65 + col_idx_max)
        
        # Only calc delta if count > 1? Or always?
        # Screenshot shows GHI has no delta/deviation.
        # Let's use IF(COUNT(...)>1, ..., "")
    ...
    worksheet.write(summary_start_row+2, col_idx_vals_start, "% difference from Min GHI", fmt_bold_border)
    ...
    # We need to find the Min of rows where Type="GHI".
    # Assuming "Type" is Col B (Index 1) and Values are Col C... (Index 2...)
    # We can use Array formula MIN(IF(B:B="GHI", C:D)) but easier:
    # Just take % of the Min calculated above? The screenshot says "% difference from Min GHI".
    # This implies (Range W/m^2) / (Global Min GHI).
    # Since the table mixes POA and GHI, Global Min usually IS the Min GHI (unless POA is wildly lower).
    # Let's use the Global Min we found in step 1 as the denominator.
