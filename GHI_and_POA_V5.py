...
def prep_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Standardizes timestamps. Assumes logger data is in fixed Standard Time (EST).
    """
    df = df.copy()
...
def plot_median_ghi_poa_overlay(df, day, noon, start_time, end_time):
    ...
    ax.set_title(f"Median GHI & POA Overlay • {day} ({start_str} - {end_str}) • Noon: {noon_str_title}", fontsize=13)
    ax.set_ylabel("Irradiance [W/m²]")
    ax.set_xlabel("Time (HH:MM)")
    
    # 2. X-Axis Tick Formatting Optimization
    # Use standard 15-minute intervals aligned to the hour (00, 15, 30, 45)
    ax.xaxis.set_major_locator(mdates.MinuteLocator(byminute=[0, 15, 30, 45]))
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
    # Add minor ticks every 5 minutes for better resolution
