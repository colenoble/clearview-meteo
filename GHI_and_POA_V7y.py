...
def main():
    ...
    def export_analysis(t1, t2, filename_suffix):
        if t1 is None or t2 is None:
            print(f"  -> Skipping output {filename_suffix}: Timestamp missing.")
            return
    ...
    if EXPORT_ESTIMATED_NOONS:
        ...
        # POA (Min)
        t1_poa = df1_medians.loc[df1_medians["Median_POA"].idxmin(), "t_stamp_dt"] if "Median_POA" in df1_medians.columns and not df1_medians["Median_POA"].isna().all() else None
        t2_poa = df2_medians.loc[df2_medians["Median_POA"].idxmin(), "t_stamp_dt"] if "Median_POA" in df2_medians.columns and not df2_medians["Median_POA"].isna().all() else None
