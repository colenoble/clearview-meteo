...
# Export Options
EXPORT_ESTIMATED_NOONS = True  # If True, generates separate files for Calculated, Est GHI, and Est POA.
...
def calculate_medians(df, ghi_cols, poa_cols):
    """
    Calculates the row-wise median for GHI and POA columns.
    Returns a new dataframe with just t_stamp_dt and the medians.
    """
    ...
    if avail_ghi:
        df_med["Median_GHI"] = df[avail_ghi].median(axis=1)
    if avail_poa:
        df_med["Median_POA"] = df[avail_poa].median(axis=1)
...
def plot_median_ghi_poa_overlay(df, day, noon, start_time, end_time):
    ...
    # 2. Add Vertical Line for Calculated Solar Noon
    ...
    # 3. Find and Add Vertical Lines for Empirical Peaks
    if "Median_GHI" in df.columns and not df["Median_GHI"].isna().all():
        ...
        max_ghi_str = max_ghi_time.strftime("%H:%M")
        ax.axvline(max_ghi_time, color="tab:blue", linestyle=":", linewidth=2, label=f"Est GHI Noon ({max_ghi_str})")
    ...
    if "Median_POA" in df.columns and not df["Median_POA"].isna().all():
        ...
        max_poa_str = max_poa_time.strftime("%H:%M")
        ax.axvline(max_poa_time, color="tab:orange", linestyle=":", linewidth=2, label=f"Est POA Noon ({max_poa_str})")
    ...
    # Standardize X-axis limits to match configuration
...
def write_excel_sheet(wb, sheet_name, date_val, records):
    ...
    headers = ["Type", "Sensor", "Solar Noon Est (EST)", "Irradiance (W/m²)", "Tilt (°)"]
    ws.append(headers)
...
def write_median_timeseries_sheet(wb, sheet_name, df, noon):
    """
    Writes the full time-series of Median_GHI and Median_POA for the day.
    """
    if sheet_name in wb.sheetnames:
        del wb[sheet_name]
    ws = wb.create_sheet(sheet_name)
    
    noon_str = pd.to_datetime(noon).round("1min").strftime("%H:%M:%S")
    ws.append(["Calculated Solar Noon:", noon_str])
    ws.append([])
...
def create_full_excel_report(output_path, records1, day1, records2, day2, df_median, noon2):
    try:
        ...
        # Sheet 3: Time Series (Usually focused on Day 2 / Post-Remediation)
...
def main():
    ...
    # Extract unique station names safely (ignoring non-string columns if any)
    stations = sorted(list(set([c.split("/")[0] for c in src1["poa_cols"]])))
    
    for st in stations:
        # Filter columns for specific station
    ...
    def get_records(row, cols, m_type):
        ...
        for c in cols:
            if c in row.index:
                ...
                # Attempt to find corresponding TILT column
