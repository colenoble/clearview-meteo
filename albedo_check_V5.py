...
def process_and_export(df_valid, output_folder, file_prefix, plot_title_prefix):
    ...
    # Create subfolder for the Intra-Station plots
    intra_plot_dir = output_folder / 'IntraStation_Effective_Albedo'
    intra_plot_dir.mkdir(exist_ok=True)
    ...
    # Setup for dynamic intra-station subplots
    ...
    for date, group in df_filt.groupby('Date'):
        ...
        day_dev_record = {'Date': date}

        # ==========================================
        # PLOT 1: Standard/Mean Albedo Comparison 
        # ==========================================
        ...
        if plotted_any:
            ...
            std_text2 += f"---\nCross-Station Std Dev: {std2*100:.2f}%" if pd.notna(std2) else "---\nCross-Station Std Dev: N/A"
        ...
        # ==========================================
        # PLOT 2: Intra-Station Effective Albedo
        # ==========================================
        fig_intra, axes_intra = plt.subplots(n_rows, n_cols, figsize=(15, 6 * n_rows), sharex=True)
        if n_stations == 1:
            axes_intra = np.array([axes_intra])
        axes_flat = axes_intra.flatten()
        ...
        for i, station in enumerate(STATIONS):
            ...
            has_1 = eff1_col in group.columns
            has_2 = eff2_col in group.columns
            
            if not has_1 and not has_2:
                day_dev_record[f'MET{station}'] = np.nan
                ax.set_visible(False)
                continue
                
            day_data1 = group[eff1_col].dropna() if has_1 else pd.Series(dtype=float)
            day_data2 = group[eff2_col].dropna() if has_2 else pd.Series(dtype=float)

            day_data1 = day_data1[(day_data1.index >= win_start) & (day_data1.index <= win_end)]
            day_data2 = day_data2[(day_data2.index >= win_start) & (day_data2.index <= win_end)]
            ...
            if not day_data1.empty:
                idx1 = day_data1.index[np.argmin(np.abs(day_data1.index - noon_time))]
                val1 = day_data1.loc[idx1]
                ax.plot(day_data1.index, day_data1, marker='.', linestyle='-', color='tab:blue', label='Sensor 1')
                intra_plotted = True

            if not day_data2.empty:
                idx2 = day_data2.index[np.argmin(np.abs(day_data2.index - noon_time))]
                val2 = day_data2.loc[idx2]
                ax.plot(day_data2.index, day_data2, marker='.', linestyle='-', color='tab:orange', label='Sensor 2')
                intra_plotted = True

            # Calculate and plot the Deviation Percentage 
            mean_dev = np.nan
            mean_dev_str = ""
            has_twin = False
            
            if not day_data1.empty and not day_data2.empty:
                aligned = pd.concat([day_data1, day_data2], axis=1, keys=['s1', 's2']).dropna()
                if not aligned.empty:
                    mean_vals = (aligned['s1'] + aligned['s2']) / 2
                    
                    dev_pct = (np.abs(aligned['s1'] - aligned['s2']) / mean_vals.replace(0, np.nan)) * 100
                    
                    ax_twin = ax.twinx()
                    ax_twin.plot(dev_pct.index, dev_pct, linestyle=':', color='crimson', alpha=0.7, label='Abs Dev (%)')
                    ax_twin.set_ylabel('Deviation (%)', color='crimson', fontsize=9)
                    ax_twin.tick_params(axis='y', labelcolor='crimson', labelsize=8)
                    ax_twin.set_ylim(bottom=0)
                    has_twin = True
                    
                    mean_dev = dev_pct.mean()
                    mean_dev_str = f" | Window Mean Dev: {mean_dev:.2f}%"

            # Store the daily mean deviation (divide by 100 so Excel can format as percentage natively)
            day_dev_record[f'MET{station}'] = (mean_dev / 100.0) if pd.notna(mean_dev) else np.nan

            # Create Dynamic Title with Deviation stats
            title_str = f"MET{station}"
            if pd.notna(val1) and pd.notna(val2):
                noon_mean = (val1 + val2) / 2
                diff_pct = (abs(val1 - val2) / noon_mean) * 100 if noon_mean != 0 else np.nan
                title_str += f" | Noon Dev: {diff_pct:.2f}%" if pd.notna(diff_pct) else ""
            title_str += mean_dev_str

            ax.set_title(title_str, fontsize=10)
            ax.axvline(solar_noon, color='gray', linestyle='--', alpha=0.6, label='Solar Noon')
            ax.grid(True, linestyle=':', alpha=0.6)
            ax.set_ylabel('Effective Albedo Ratio')
        ...
        monthly_dev_records.append(day_dev_record)

        for j in range(i + 1, len(axes_flat)):
            axes_flat[j].set_visible(False)

        if intra_plotted:
            fig_intra.suptitle(f"Intra-Station Sensor Variance: Individual RPOA vs Mean Station POA - {date}", fontsize=14)
            plt.tight_layout()
            intra_plot_path = intra_plot_dir / f"IntraStation_Eff_Albedo_{date}.png"
            fig_intra.savefig(intra_plot_path, dpi=300, bbox_inches='tight')
...
def main():
    ...
    # Export the Global Summary
    if all_deviations:
        summary_df = pd.concat(all_deviations, ignore_index=True)
        summary_df.set_index('Date', inplace=True)
        
        summary_path = OUTPUT_DIR / "Overall_IntraStation_Deviation_Summary.xlsx"
        with pd.ExcelWriter(summary_path, engine='xlsxwriter') as writer:
            summary_df.to_excel(writer, sheet_name='Daily Mean Deviations')
            
            workbook = writer.book
            ws = writer.sheets['Daily Mean Deviations']
            pct_fmt = workbook.add_format({'num_format': '0.00%'})
            
            # Format Excel Columns automatically
            ws.set_column(0, 0, 15) # Expand Date column
            for col_num, col_name in enumerate(summary_df.columns, start=1):
                ws.set_column(col_num, col_num, 15, pct_fmt)
                
        print(f"\nExported Overall Deviation Summary to: {summary_path}")
