...
# --- Configuration ---
INPUT_DIR = Path(r"T:\Shared\93 Solar USA\Projects\9300001\Internal\14. Commissioning and Testing\Performance Testing Share Folder")
...
# Strict, case-sensitive search target
TARGET_FILE_SUBSTRING = "MET_" 
...
# New Configuration: Plot Actual Albedo (ARRAY_ALBEDO) for dates >= Threshold
...
def process_and_export(df_valid, output_folder, file_prefix, plot_title_prefix):
    ...
    exclusion_stats = {}
    # Include ARRAY_ALBEDO in the filtering and export logic if it exists
    export_cols = [c for c in df_valid.columns if 'Calc_Albedo' in c or 'Eff_Albedo' in c or 'ARRAY_ALBEDO' in c]
    ...
    with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
        ...
        for col_num, col_name in enumerate(df_filt.columns, start=1):
            if "Albedo" in col_name or "ARRAY_ALBEDO" in col_name:
    ...
    for day_idx, (date, group) in enumerate(grouped_by_date, start=1):
        ...
        # Check if Date exceeds threshold for using Actual Albedo
        use_actual = False
        if ACTUAL_ALBEDO_DATE_THRESHOLD and date >= pd.to_datetime(ACTUAL_ALBEDO_DATE_THRESHOLD).date():
            use_actual = True
        ...
        noon_vals1 = []
        noon_vals2 = [] # Will hold up to 8 values (2 per station)
        ...
        for i, station in enumerate(STATIONS):
            ...
            actual_col = f'MET{station}/ARRAY_ALBEDO'
            
            # Decide Top Plot column based on threshold and availability
            if use_actual and actual_col in group.columns:
                top_plot_col = actual_col
                top_plot_label = f"MET{station} (Actual)"
            else:
                top_plot_col = calc_col
                top_plot_label = f"MET{station} (Calc)"
                
            eff1_col = f'MET{station}_Eff_Albedo_1' 
            eff2_col = f'MET{station}_Eff_Albedo_2' 
            
            day_data1_full = group[top_plot_col] if top_plot_col in group.columns else pd.Series(dtype=float)
            ...
            if not day_data_eff1.empty:
                # Sensor 1: Dotted Line with Dot Marker
        ...
        # Record Daily Overall Standard Deviation for Global Summary
        ...
        if plotted_any:
            ...
            if use_actual:
                ax1.set_title(f'Actual High-Resolution Albedo and Mean GHI - {date}\n(Filtered: {START_TIME}-{END_TIME}, GHI>{MIN_GHI})')
                ax1.set_ylabel('Actual Albedo (ARRAY_ALBEDO)')
            else:
...
def main():
    ...
    # 1. Targeted File Search (Strict, Case-Sensitive Search)
    if TARGET_FILE_SUBSTRING:
        files = [f for f in INPUT_DIR.rglob('*.csv') if TARGET_FILE_SUBSTRING in f.name]
    else:
        files = list(INPUT_DIR.rglob('*.csv'))
        
    total_files = len(files)
    if not files:
        print(f"No CSV files containing '{TARGET_FILE_SUBSTRING}' (case-sensitive) found in {INPUT_DIR} or subfolders.")
        return
        
    print(f"\n[INFO] Found {total_files} target files. Reading and resolving overlaps by Date...")
    
    # 2. Resolution-Aware Deduplication per Date
    best_data_for_date = {}
    best_res_for_date = {}
    ...
    for i, f in enumerate(files, start=1):
        ...
        try:
            ...
            if TIMESTAMP_COL not in file_df.columns:
                continue
                
            file_df[TIMESTAMP_COL] = pd.to_datetime(file_df[TIMESTAMP_COL])
            file_df = file_df.sort_values(TIMESTAMP_COL).set_index(TIMESTAMP_COL)
    ...
    if not best_data_for_date:
        print("\n[ERROR] No valid dates found after processing files.")
        return

    print(f"[DEBUG] Assembled {len(best_data_for_date)} unique days of highest-resolution data.")
