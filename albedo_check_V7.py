...
ACTUAL_ALBEDO_TEMPLATES = ["MET{}/ARRAY_ALBEDO"] 
...
def main():
    ...
    possible_files = [f for f in INPUT_DIR.rglob('*.csv') if LEGACY_FILE_IDENTIFIER in f.name or HIGH_RES_FILE_IDENTIFIER in f.name]
        
    total_files = len(possible_files)
    if not possible_files:
        print(f"[ERROR] No CSV files found matching '{LEGACY_FILE_IDENTIFIER}' or '{HIGH_RES_FILE_IDENTIFIER}'.")
        return
    ...
    for i, f in enumerate(possible_files, start=1):
        ...
        try:
            ...
            for date, group in file_df.groupby(file_df.index.date):
                ...
                if is_after_threshold and HIGH_RES_FILE_IDENTIFIER and HIGH_RES_FILE_IDENTIFIER not in f.name:
                    continue 
    ...
    if not best_data_for_date:
        print("\n[ERROR] No valid dates found after applying threshold parsing rules.")
        return
